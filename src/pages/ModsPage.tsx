...
import { AlertTriangle, CheckCircle2, ChevronDown, FileBox, FolderCog, FolderOpen, LayoutGrid, List, MoreHorizontal, Search, SlidersHorizontal, Wrench, EyeOff } from 'lucide-react';
...
export function ModsPage() {
  const { settings, scan, packs, runScan, updatePack, updateSettings, addActivity } = useApp();
  ...
  async function ignoreFileLocation(file: ModFile) {
    if (!settings) return;
    const location = file.relativePath.replace(/[\\/][^\\/]+$/, '');
    if (!location || location === file.relativePath) return;
    await updateSettings({ ignoredUncategorizedLocations: [...new Set([...(settings.ignoredUncategorizedLocations ?? []), location])] });
    await runScan();
  }
