...
export function SettingsPage() {
  ...
  async function browseInstallFolder() {
    const selected = await api.chooseFolder('Choose where Plumbuddy should install updates', draft!.appInstallFolder);
    if (!selected) return;
    setDraft(value => value ? { ...value, appInstallFolder: selected } : value);
    await updateSettings({ appInstallFolder: selected });
    await addActivity({ type: 'settings', title: 'Application install folder changed', detail: `${selected}\\Plumbuddy.exe` });
    setSaved(true); window.setTimeout(() => setSaved(false), 2200);
  }
  ...
  return <>
    ...
    <section className="settings-card update-settings"><div className="settings-heading"><span><Download size={19} /></span><div><h2>App install & updates</h2><p>Choose where Plumbuddy lives, then updates will install there as Plumbuddy.exe.</p></div></div><div className="app-install-target"><div><strong>Install Plumbuddy in this folder</strong><small>Updates download here first, replace <code>Plumbuddy.exe</code>, then reopen from this location.</small><code>{`${draft.appInstallFolder}\\Plumbuddy.exe`}</code>{updateError && <p className="form-error">{updateError}</p>}</div><button className="button ghost" onClick={() => void browseInstallFolder()}><FolderOpen size={15} /> Change folder</button></div><div className="update-row"><div><strong>Latest GitHub release</strong><small>Check for the newest portable build and install it into your selected application folder.</small></div><button className="button primary" disabled={checkingUpdate} onClick={() => void checkUpdates()}><RefreshCw size={15} className={checkingUpdate ? 'spin' : ''} /> {checkingUpdate ? 'Checking...' : 'Check for app updates'}</button></div></section>
    ...
    {updateInfo && <Modal title={updateInfo.updateAvailable ? 'App update available' : 'Plumbuddy is up to date'} subtitle={`Current ${updateInfo.currentVersion} · Latest ${updateInfo.latestVersion}`} onClose={() => setUpdateInfo(null)}>
      <div className="update-modal">
        ...
        {updateInfo.updateAvailable ? <div className="modal-note"><Download size={17} /><span>{updateInfo.assetName ? `Plumbuddy will download the update, install it as ${draft.appInstallFolder}\\Plumbuddy.exe, close itself, and restart from that folder.` : 'A release exists, but no portable EXE asset was attached yet.'}</span></div> : <div className="modal-note"><ShieldCheck size={17} /><span>You already have the latest published version.</span></div>}
